...
CHECK=$(git diff --name-only HEAD * | grep -v -f $patterns | sort -u)

# If no files are checked out, get list of files from last commit
if [[ -z "$CHECK" ]]; then
    CHECK=$(git diff-tree --no-commit-id --name-only -r $(git log -2 --pretty=format:"%h") \
