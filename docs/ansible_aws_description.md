...
This document focused on the explanation of files related to ansible agent
which you need to alterate .If you run your custom network
for deploying fabric on AWS using ansible .
There are two aws related files in ansible agent directory .
One is root file located in main ansible agent directory
while other is located in vars directory.
...
If you define your custom fabric configuration file then you need to 
change this section according to your filename
...
AWS secret access token from your aws account .
Once you generated those keys , replace that`AWS Access Key Id` with 
your key .
...
Normally free aws account have limit of `t2.micro` instances 
which is not enough for running fabric network .

Next section is related to public key /private key section . 
Ansible will store public key and private key in your
local ssh directory so that later on you can ssh’ed into your ubuntu instances
from your terminal .

One thing you need to take care is `ssh_key_name` . You should generate and download your
ssh key-pairs from your aws account here 
