...
In MacOS, Docker cannot mount local path from host by default. Hence for mongo container data volume, users need to create the `/opt/cello` path locally, and allow it to be mounted by specifying in the preference of [Docker for MacOS](https://docs.docker.com/docker-for-mac/install/).
