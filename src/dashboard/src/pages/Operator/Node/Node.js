...
const RegisterUserForm = Form.create()(props => {
  const { registerUserFormVisible, form, handleSubmit, handleModalVisible, registeringUser, targetNodeId } = props;
  const onSubmit = () => {
  ...
  return (
    <Modal
      destroyOnClose
      title={
        <FormattedMessage id="app.operator.node.table.operation.registerUser" defaultMessage="Register User" />
      }
...
class Node extends PureComponent {
  ...
  operationForNodeCallback = data => {
    message.success(
      formatMessage({
        ...
        defaultMessage: `${data.payload.message.substring(0, 1).toUpperCase() + data.payload.message.substring(1)} Node Successful.`,
      })
    );
    this.queryNodeList();
  };
  ...
  render() {
    ...
    const menu = record => (
      <Menu>
        <Menu.Item>
          <a onClick={() => this.operationForNode('start', record)}><FormattedMessage id="app.operator.node.table.operation.start" defaultMessage="Start" /></a>
        </Menu.Item>
        <Menu.Item>
          <a onClick={() => this.operationForNode('stop', record)}><FormattedMessage id="app.operator.node.table.operation.stop" defaultMessage="Stop" /></a>
        </Menu.Item>
        <Menu.Item>
          <a onClick={() => this.operationForNode('restart', record)}><FormattedMessage id="app.operator.node.table.operation.restart" defaultMessage="Restart" /></a>
        </Menu.Item>
      </Menu>
    );
    ...
    const MoreBtn = record => (
      <Dropdown overlay={menu(record)}>
        <a>
          <FormattedMessage id="app.operator.node.table.operation.more" defaultMessage="More" /> <Icon type="down" />
        </a>
      </Dropdown>
    );
