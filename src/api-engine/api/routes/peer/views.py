...
class PeerViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        query_serializer=PeerQuery,
        responses=with_common_response(
            with_common_response({status.HTTP_200_OK: PeerListSerializer})
        ),
    )
    def list(self, request, *args, **kwargs):
        ...
        """
        serializer = PeerQuery(data=request.GET)
    ...
    @swagger_auto_schema(
        request_body=PeerCreateBody,
        responses=with_common_response(
            {status.HTTP_201_CREATED: PeerIDSerializer}
        ),
    )
    def create(self, request):
        ...
        Create peer
        """
        serializer = PeerCreateBody(data=request.data)
        if serializer.is_valid(raise_exception=True):
            ...
            response = PeerIDSerializer(data={"id": str(node.id)})
            if response.is_valid(raise_exception=True):
                return Response(
    ...
    @swagger_auto_schema(
        methods=["post"],
        query_serializer=PeerOperationSerializer,
