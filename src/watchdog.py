...
def network_check_health(network_id, retries=60, period=5):
    """
    Check the chain health. If not healthy, will reset the chain

    :param network_id: id of the chain
    :param retries: how many retries before thinking not health
    :param period: wait between two retries
    :return:
    """
    network = cluster_handler.get_by_id(network_id)
    if not network:
        logger.warning("Not find chain {}".format(network_id))
        return
    if network.get("status") != NETWORK_STATUS_RUNNING:  # check running one
        return
    network_name = network.get("name")
    logger.debug("Chain {}/{}: checking health".format(network_name, network_id))

    # free or used by user, then check its health
    for i in range(retries):
        if cluster_handler.refresh_health(network_id):  # chain is healthy
            return
        else:
            logger.debug("Health Check {}: cluster {}/{} is unhealthy!".format(
                i, network_name, network_id))
            time.sleep(period)
    logger.warning("Chain {}/{} is unhealthy!".format(network_name, network_id))
    # only reset free chains
    if cluster_handler.get_by_id(network_id).get("user_id") == "":
        logger.info("Timeout....resetting free unhealthy chain {}/{}".format(
            network_name, network_id))
        cluster_handler.reset_free_one(network_id)
