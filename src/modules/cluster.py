...
class ClusterHandler(object):
    ...
    def create(self, name, host_id, config, start_port=0,
        ...
        peers_ports, ca_mapped_ports, orderer_service_ports, explorer_mapped_port, mapped_ports = \
            {}, {}, {}, {}, {}
        port_pos = 0
        ...
        logger.debug("org num list {} peer_num_end {}".format(org_num_list, peer_num_end))
        ...
        env_mapped_ports = dict(((k+'_port').upper(), str(v)) for (k, v) in mapped_ports.items())
        ...
        containers = self.cluster_agents[worker.type]\
            .create(cid, mapped_ports, self.host_handler.schema(worker), config=config, user_id=user_id)
    ...
    def delete(self, id, record=False, forced=False):
        ...
        host_id, worker_api, network_type, consensus_plugin, cluster_size = \
            ...
            c.consensus_plugin if c.consensus_plugin else CONSENSUS_PLUGINS_FABRIC_V1[0], \
    ...
    def apply_cluster(self, user_id, condition={}, allow_multiple=False):
        ...
        cluster = ClusterModel.objects(user_id=SYS_USER,
                                       network_type__icontains=condition.get("apply_type"),
                                       size=condition.get("size", 0), health="OK").first()
        if cluster:
    ...
    def find_free_start_ports(self, host_id, number):
        ...
        #     clusters_valid))
        ports_existed = [service.port for service in ServicePort.objects(cluster__in=clusters_exists)]
