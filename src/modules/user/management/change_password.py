...
class ChangePassword(Resource):
    ...
    def post(self, user_id):
        ...
        origin_password, new_password = args["origin_password"], args["new_password"]
        ...
        password = bcrypt.hashpw(origin_password.encode('utf8'), bytes(salt.encode()))
        if not password.decode() == user.dbUser.password:
            return {"error": "Invalid origin password", "success": False}, 400
        new_password = bcrypt.hashpw(new_password.encode('utf8'), bytes(salt.encode()))
