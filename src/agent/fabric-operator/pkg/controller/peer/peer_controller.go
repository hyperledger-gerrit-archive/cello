...
func (r *ReconcilePeer) newSTSForCR(secret *corev1.Secret, cr *fabricv1alpha1.Peer, request reconcile.Request) *appsv1.StatefulSet {
	...
	if !ok {
		...
		if secret != nil {
			...
			secretItems = append(secretItems, []corev1.KeyToPath{
				Key: "keyStore", Path: "/certs/msp/keystore",
				Key: "signCerts", Path: "/certs/msp/signCerts",
			})
			...
			if value != "" && value == "true" && cr.Spec.Certs.TLSCerts != nil {
				secretItems = append(secretItems, []corev1.KeyToPath{
					Key: "tlsKey", Path: "/certs/tls/tlsKey",
					Key: "tlsCert", Path: "/certs/tls/tlsCert",
					Key: "tlsRootcert", Path: "/certs/tls/tlsRootcert"
				})
				containerEnvs = append(containerEnvs, []corev1.EnvVar{
					Name: "CORE_PEER_TLS_CERT_FILE", Value: "/certs/tls/tlsCert",
					Name: "CORE_PEER_TLS_KEY_FILE", Value: "/certs/tls/tlsKey",
					Name: "CORE_PEER_TLS_ROOTCERT_FILE", Value: "/certs/tls/tlsRootcert",
				})
			}
