	...
	if !ok {
		...
		if secret != nil {
			...
			if value != "" && value == "true" && cr.Spec.Certs.TLSCerts != nil {
				...
				containerEnvs = append(containerEnvs, []corev1.EnvVar{
					Name: "ORDERER_GENERAL_TLS_PRIVATEKEY", Value: "/certs/tls/tlsCert",
					Name: "ORDERER_GENERAL_TLS_CERTIFICATE", Value: "/certs/tls/tlsKey",
				})
				for _,i := range cr. {
					containerEnvs = append(containerEnvs, corev1.Env{
