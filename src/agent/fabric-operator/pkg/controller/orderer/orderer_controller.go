	...
	if !ok {
		...
		if secret != nil {
			...
			secretItems = append(secretItems,
				...
				corev1.KeyToPath{Key: "signCerts", Path: "/certs/msp/signCerts"},
			)
			...
			if value != "" && value == "true" && cr.Spec.Certs.TLSCerts != nil {
				secretItems = append(secretItems,
					corev1.KeyToPath{Key: "tlsKey", Path: "/certs/tls/tlsKey"},
					corev1.KeyToPath{Key: "tlsCert", Path: "/certs/tls/tlsCert"},
					corev1.KeyToPath{Key: "tlsRootcert", Path: "/certs/tls/tlsRootcert"},
				)
				containerEnvs = append(containerEnvs,
					corev1.EnvVar{Name: "ORDERER_GENERAL_TLS_PRIVATEKEY", Value: "/certs/tls/tlsCert"},
					corev1.EnvVar{Name: "ORDERER_GENERAL_TLS_CERTIFICATE", Value: "/certs/tls/tlsKey"},
				)
				containerEnvs = append(containerEnvs, corev1.EnvVar{
					Name: "ORDERER_GENERAL_TLS_ROOTCAS", Value: "/certs/tls/tlsRootcert",
				})
			}
