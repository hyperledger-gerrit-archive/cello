...
class KubernetesOperation():
    ...
    def _get_config_from_params(self, k8s_params):
        ...
        k8s_config.cert_file = config.kube_config._create_temp_file_with_content(cert_content)
        k8s_config.key_file = config.kube_config._create_temp_file_with_content(key_content)

        config_content = k8s_params.get('config')
        # Use config file content to set k8s_config if it exist.
        if config_content.strip():
            config_file = config.kube_config._create_temp_file_with_content(config_content)
