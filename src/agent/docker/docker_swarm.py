...
def compose_up(name, host, mapped_ports, network_type=NETWORK_TYPES[0],
    ...
    logger.debug(
        "Compose start: name={}, host={}, mapped_port={}, config={} network_type={}".format(
            name, host.get("name"), mapped_ports, config.get_data(), network_type))
    ...
    try:
        template_path = COMPOSE_FILE_PATH + "/{}/".format(network_type) + log_type
